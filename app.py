...
<style>
    ...
    /* 3. УСКОРЕНИЕ УВЕДОМЛЕНИЙ (TOAST) */
    /* Принудительно делаем анимацию короче (3 секунды) */
    [data-testid="stToast"] {
        animation: toast-animation 3s ease-in-out forwards !important;
    }

    @keyframes toast-animation {
        0% { opacity: 0; transform: translateY(-20px); }
        10% { opacity: 1; transform: translateY(0); }
        70% { opacity: 1; } /* Начинаем исчезать чуть раньше */
        100% { opacity: 0; }
    }
</style>
...
with col1:
    ...
    # Добавил label и убрал скрытие label
    email_input = st.text_input(
        "Введите ваш email (например: name@gmail.com)", 
        key="email_input_key",
        placeholder="vash_email@gmail.com"
    )

with col2:
    # Отступ сверху, чтобы кнопка выровнялась с полем ввода (визуально)
    st.write("") 
    st.write("") 
...
if 'result' in st.session_state:
    ...
    # Мы добавили подсказку, чтобы пользователь ее увидел.
    st.code(st.session_state['result'], language="text")
    st.caption("ℹ️ Нажмите на значок 📄 в правом верхнем углу блока с текстом, чтобы скопировать список.")
